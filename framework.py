...
def get_samples(path:str):
    ...
    for s in s_list:
        samples.append(os.path.join(path, s))
    return samples
...
class Config:
    ...
    def transfer(self):
        app = self.get_app()
        with app:
            app.transfer(self.get_ds_transfer())

    def get_ds_transfer(self):
        raise Exception('get_ds_transfer() is not defined.')
        ...
        for epoch in range(cfg.epoches):
            ...
            for batch in range(batches):
                ...
                if len(ts.train_ops) == 1:
                    ...
                    self.session.run(ts.train_ops[0], feed_dict)
                else:
